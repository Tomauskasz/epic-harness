...
- **Digester** (`src/evolve/digester.rs`): compresses a session's observations into per-task `TaskDigest`s — binary outcome, ranked failure categories, implicated components, evidence excerpts, tool trajectory. Stable pipeline IDs feed the seesaw's cross-session regression tracking; fallback IDs are session-namespaced. Paper §4.3.
