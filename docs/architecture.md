...
4. **Single agent**: The evolution loop assumes a single Claude Code or Codex session. Concurrent multi-agent execution may cause observation conflicts. Skill synthesis is decoupled from the session via the pending-manifest protocol, so it survives across sessions and hosts.
