...
<div class="page">
  <div class="page-header">
    <h1>{$tStore('pageIntegrations')}</h1>
    <p>{$tStore('pageIntegrationsDesc')}</p>
  </div>

  {#if loading}
    <div class="loading">{$tStore('loadingIntegrations')}</div>
  {:else if error}
        ...
        <div class="card">
          <div style="display:flex; align-items:flex-start; justify-content:space-between; margin-bottom:0.75rem;">
            ...
            <span class="badge" style="background:{intg.installed ? 'var(--success-soft, #dcfce7)' : 'var(--surface)'}; color:{intg.installed ? 'var(--success, #22c55e)' : 'var(--text-secondary)'}; white-space:nowrap; flex-shrink:0;">
              {intg.installed ? $tStore('statusInstalled') : $tStore('statusNotInstalled')}
            </span>
          </div>
